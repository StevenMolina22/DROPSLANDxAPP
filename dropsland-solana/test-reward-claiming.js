...
async function testRewardClaiming() {
  ...
  try {
    ...
    // Test 7: Try to claim reward again (should fail - insufficient tokens)
    console.log('\n🎯 Test 7: Trying to claim reward again (should fail)...');
    ...
    try {
      ...
      console.log('❌ Second claim should have failed but succeeded');
    } catch (error) {
      console.log('✅ Second claim correctly failed (insufficient tokens)');
      console.log('Error:', error.message);
    }

    // Test 8: Mint more tokens and claim reward again
    console.log('\n🎯 Test 8: Minting more tokens and claiming reward again...');
    
    // Mint 50 more tokens
    const secondMintTx = await program.methods
      .mintSoulboundTokens(
    ...
    console.log('Transaction:', secondMintTx);

    // Check balance after second mint
    const balanceAfterSecondMint = await getAccount(provider.connection, tokenAccount);
    console.log('Token balance after second mint:', balanceAfterSecondMint.amount.toString());
    console.log('Expected: 125 tokens (75 + 50)');
    ...
    console.log('✅ Second reward claim successful');
    console.log('Transaction:', secondClaimTx);

    // Check final balance
    const finalBalance = await getAccount(provider.connection, tokenAccount);
    console.log('Final token balance:', finalBalance.amount.toString());
    console.log('Expected: 100 tokens (125 - 25 burned)');
